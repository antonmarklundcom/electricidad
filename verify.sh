...
while IFS=$'\t' read -r slug path; do
  ...
  # dies of SIGPIPE — which would fail every page that actually passes. The
  # same holds for `printf "$html" | grep -q` once a page outgrows the pipe
  # buffer, so grep reads a here-string instead of a pipe.
  html=$(curl -s "${BASE}${path}")
